...
public class Spawner : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        for (int i = 0; i < _repeatCount; i++)
        {
            ...
            GenerateRandomLine(_wallSpawnPoints, _wallTemplate.gameObject, _wallSpawnChance, _distanceBetweenFullLine, _distanceBetweenFullLine / 2f);
            GenerateFullLine(_blockSpawnPoints, _blockTemplate.gameObject);
            MoveSpawner(_distanceBetweenRandomLine);
            GenerateRandomLine(_wallSpawnPoints, _wallTemplate.gameObject, _wallSpawnChance, _distanceBetweenRandomLine, _distanceBetweenRandomLine / 2f);
    ...
    private void GenerateRandomLine(SpawnPoint[] spawnPoints, GameObject generatedElement, int spawnChance, float scaleY = 0.05f, float offsetY = 0)
    {
        for (int i = 0; i < spawnPoints.Length; i++)
        {
            if ((Random.Range(0, 100) < spawnChance))
            {
                var element = GenerateElement(spawnPoints[i].transform.position, generatedElement, offsetY);
